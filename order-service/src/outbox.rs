...
/// Polls unpublished outbox rows and ships them to Kafka. At-least-once by construction: a crash
/// between the Kafka ack and the `mark_outbox_published` commit will redeliver on the next poll,
/// so every consumer downstream must be idempotent.
