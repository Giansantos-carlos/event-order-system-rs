...
/// `<topic>.DLT` topic so one poison message can't block the partition forever, mirroring
/// Spring Kafka's DefaultErrorHandler + DeadLetterPublishingRecoverer.
...
pub async fn run_payment_events_listener(pool: PgPool, bootstrap_servers: String) {
    ...
    if let Err(e) = consume_loop(&bootstrap_servers, topics::PAYMENT_EVENTS, "order-service", handler).await {
        tracing::error!("payment-events listener crashed: {e:?}");
    }
}

pub async fn run_inventory_events_listener(pool: PgPool, bootstrap_servers: String) {
    let handler = move |envelope: EventEnvelope| {
        let pool = pool.clone();
        async move { saga::handle_inventory_event(&pool, envelope).await }
    };
    if let Err(e) = consume_loop(&bootstrap_servers, topics::INVENTORY_EVENTS, "order-service", handler).await
    {
        tracing::error!("inventory-events listener crashed: {e:?}");
    }
}
