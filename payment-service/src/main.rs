...
async fn main() -> anyhow::Result<()> {
    ...
    let db_name = std::env::var("DB_NAME").unwrap_or_else(|_| "payment_db".into());
    ...
    let failure_rate: f64 = std::env::var("PAYMENT_SIMULATED_FAILURE_RATE")
        .ok()
    ...
    tokio::spawn(kafka::run_order_events_listener(pool.clone(), bootstrap_servers.clone(), failure_rate));
    ...
    let port: u16 = std::env::var("SERVER_PORT").ok().and_then(|p| p.parse().ok()).unwrap_or(8082);
