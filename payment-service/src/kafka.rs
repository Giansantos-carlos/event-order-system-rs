...
pub async fn run_order_events_listener(pool: PgPool, bootstrap_servers: String, failure_rate: f64) {
    let handler = move |envelope: EventEnvelope| {
        let pool = pool.clone();
        async move { saga::handle_order_event(&pool, envelope, failure_rate).await }
    };
    if let Err(e) = consume_loop(&bootstrap_servers, topics::ORDER_EVENTS, "payment-service", handler).await {
